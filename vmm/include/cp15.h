...
#define get_hsb(x) ( ((x)>>24) & 0xFF )
#define get_lsb(x) ( ((x)>>16) & 0xFF )
#define get_crn(x) ( ((x)>>12) & 0xF )
#define get_opc1(x) ( ((x)>>8) & 0xF )
#define get_crm(x) ( ((x)>>4) & 0xF )
#define get_opc2(x) ( ((x)>>0) & 0xF )

typedef enum cp15_reg_names64
{
...
/*
#define asm_cp15_write(data, crn, opc1, crm, opc2)\
    {\
        __asm__ __volatile__ (\
                              "mcr p15, %0, %1, c" #crn ", c" #crm ", %2"\
                              :\
                              : "M"(opc1), "r"(data), "M"(opc2)\
                             );\
    }
    */
#define asm_cp15_write(data, crn, opc1, crm, opc2)\
    {\
        __asm__ __volatile__ (\
                              "mcr p15, " #opc1 ", %0, c" #crn ", c" #crm ", " #opc2 \
                              :\
                              : "r"(data)\
                             );\
    }

#define asm_cp15_write64(data_h, data_l, crm, opc1)\
    {\
        __asm__ __volatile__ (\
                              "mcrr p15, " #opc1 ", %0, %1, c" #crm \
                              :\
                              : "r"(data_l), "r"(data_h)\
                             );\
    }

#define asm_cp15_read(data, crn, opc1, crm, opc2)\
    {\
        __asm__ __volatile__ (\
                              "mrc p15, #" #opc1 ", %0, c" #crn ", c" #crm ", #" #opc2 \
                              : "=r"(data)\
                             );\
    }

#define asm_cp15_read64(data_h, data_l, crm, opc1)\
    {\
        __asm__ __volatile__ (\
                              "mrrc p15, " #opc1 ", %0, %1, c" #crm \
                              : "=r"(data_l), "=r"(data_h)\
                             );\
    }
